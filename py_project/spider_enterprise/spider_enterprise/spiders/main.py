...
class DmozSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for ddic in areadic:
            ...
            yield Request(next_url1, meta={'area':ddic}, callback = self.parseAllPage)
            ...
            yield Request(next_url2, meta={'area':ddic}, callback = self.parseAllPage)
            ...
            yield Request(next_url3, meta={'area':ddic}, callback = self.parseAllPage)
            ...
            yield Request(next_url4, meta={'area':ddic}, callback = self.parseAllPage)
    ...
    def parseAllPage(self, response):
        ...
        for p in xrange(maxpage):
            ...
            if goto:
                yield Request(next_url, meta={'area':response.meta['area']}, callback = self.parsePlayInfo)
            else:
    ...
    def parsePhoneNum(self, response):
        ...
        g1          =  re.search('联系',   text)
        g2          =  re.search('手机',   text)
        g3          =  re.search('phone',  text)
        g4          =  re.search('号码',   text)
        g5          =  re.search('电话',   text)
        ...
        if sign1 or sign2:
            ...
            if item['name'] not in self.legalButNotNum:
                self.addNotExistName(item['name'])
                self.legalButNotNum[item['name']] = -1
